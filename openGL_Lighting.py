...
class WireframeViewer(wf.WireframeGroup):
            ...
            if self.displayFaces:
                for (face, colour) in wireframe.sortedFaces():
                    ...
                    if towards_us > 0:
                        ...
                        R = (2 * np.dot(normal, self.light_vector) * normal) - self.light_vector
                        ...
                        #if np.(reflectance, normal) < 0
                         #only diffuse
                        elif np.dot(R, self.view_vector) < 0:
                            specular = np.array([0,0,0])
                            diffuse = m_diffuse * self.light_color * colour * np.dot(normal,self.light_vector)
                        else:
                            diffuse = m_diffuse * self.light_color * colour * np.dot(normal,self.light_vector)
                            specular = m_specular * self.light_color * colour * (np.dot(self.view_vector, R) ** gloss)

						#Once you have implemented diffuse and specular lighting, you will want to include them here
                        light_total = diffuse + ambient + specular
    ...
    def keyEvent(self, key):
        ...
        #Your code here
        if key == pygame.K_q:
            rotate = np.array([[np.cos(-0.1), -np.sin(-0.1), 0],
        ...
        if key == pygame.K_e:
            rotate = np.array([[np.cos(0.1), -np.sin(0.1), 0],
                                ...
                                [0,0,1]])
        if key == pygame.K_a:
            rotate = np.array([[np.cos(0.1), 0, np.sin(0.1)],
                                [0,1,0],
        ...
        if key == pygame.K_d:
            rotate = np.array([[np.cos(-0.1), 0, np.sin(-0.1)],
                                [0,1,0],
                                [-np.sin(-0.1), 0, np.cos(-0.1)]])
        self.light_vector = np.matmul(rotate, self.light_vector)
